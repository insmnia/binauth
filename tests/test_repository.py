...
class TestAsyncPermissionRepository:
    ...
    async def test_get_user_permission_empty(self, async_session, manager):
        ...
        repo: AsyncPermissionRepository[int] = AsyncPermissionRepository(
            async_session, manager
        )
    ...
    async def test_has_all_permissions(self, async_session, manager, task_permissions):
        ...
        assert await repo.has_all_permissions(
            1, "tasks", [Actions.CREATE, Actions.READ]
        ) is True
        assert await repo.has_all_permissions(
            1, "tasks", [Actions.CREATE, Actions.DELETE]
        ) is False
    ...
    async def test_has_any_permission(self, async_session, manager, task_permissions):
        ...
        assert await repo.has_any_permission(
            1, "tasks", [Actions.CREATE, Actions.DELETE]
        ) is True
        assert await repo.has_any_permission(
            1, "tasks", [Actions.UPDATE, Actions.DELETE]
        ) is False
